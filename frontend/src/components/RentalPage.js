...
function RentalPage() {
  ...
  const handleSubmit = async () => {
    ...
    const data = {
      ...
      totalPrice: vehicle.price_per_day  // Consistent with API field
    };
  ...
  if (!vehicle) {
    return <p className="text-center py-10 text-[#5A827E] text-xl">No vehicle selected.</p>;
  }
  ...
  return (
    <div className="min-h-screen bg-[#FAFFCA] py-12 px-4 md:px-10 flex justify-center">
      <div className="bg-white shadow-md border border-[#B9D4AA] rounded-2xl max-w-xl w-full p-8">
        ...
        />
        <p className="text-lg font-semibold text-[#3F7C3E] mb-6">Price per Day: ₹{vehicle.price_per_day}</p>
